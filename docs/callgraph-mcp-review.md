...
| 8 | USR-based node identity | F8 | ✅ done (3 PRs per the design's phasing) — fixtures-first gate (#34, expected-fail), identity core (#35: USR-keyed nodes, byName_ resolution, compound call-site key, snapshot v6; 938-TU: bake +2–4%, nodes 57k→95k, RSS +15%), MCP disambiguation contract (ambiguous names → candidates, usr-precise queries; verified live: the two process() overloads return disjoint caller sets) |
...
| 11 | Snapshot bulk-load fast path | F9 | ✅ done — v5 id-preserving format: interner tables serialized in id order, node/edge/context records as raw ids, bulk installStrings + direct install (no per-record re-interning; the old loader also replayed nodes/edges once PER CONTRIBUTOR). Measured section split before: nodes 2.5 s + edges 2.5 s + contexts 6.1 s; after: total load 2.3–2.6 s (~4.5×), counts identical, post-load reindex_tu exact. Snapshot 285 → 322 MB (interner duplication + inline strings — accepted). Measurement note: cross-hours absolute timings on this box drift ±40% with thermal state — compare arms interleaved in one window (the item-10 "bake regression" was this artifact) | — map pre-reserving landed (~4%: the cost is string copies + re-interning, i.e. item 10 territory), and unchanged warm starts skip the 3.1s snapshot re-save (14.9s → 11.6s total warm start). The remaining 8s load wants the id-preserving format that falls out of item 10 |
