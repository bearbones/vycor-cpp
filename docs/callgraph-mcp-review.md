...
| 11 | Snapshot bulk-load fast path | F9 | still open after item 10: load remains ~8 s at 938 TUs (re-interning per context); the id-preserving pool-install format is the remaining lever. Measurement note: cross-hours absolute timings on this box drift ±40% with thermal state — compare arms interleaved in one window (the item-10 "bake regression" was this artifact) | partially done — map pre-reserving landed (~4%: the cost is string copies + re-interning, i.e. item 10 territory), and unchanged warm starts skip the 3.1s snapshot re-save (14.9s → 11.6s total warm start). The remaining 8s load wants the id-preserving format that falls out of item 10 |
